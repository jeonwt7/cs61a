...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    lst_d = zip(centroids, [distance(location,cen) for cen in centroids])
    return min(lst_d, key=lambda pair:pair[1])[0]
    # END Question 3
...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    locations = [restaurant_location(x) for x in restaurants]
    c_ordered = [find_closest(x, centroids) for x in locations]
    return group_by_first(zip(c_ordered, restaurants))
    # END Question 4
