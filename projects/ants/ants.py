...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and insect.real: # and self.real is True:
                return None
            # END Problem 13
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        return self.container and not self.ant and other.container is False
        # END Problem 9
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while self.min_range != cur_range:
            cur_place = cur_place.entrance
            cur_range += 1
        while not cur_place.bees and cur_place.entrance is not hive and cur_range <= self.max_range:
            cur_place = cur_place.entrance
            cur_range += 1
        if cur_range <= self.max_range:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        for b in self.place.bees:
            b.armor -= self.damage
        for i in range(len(self.place.bees)-1,-1,-1):
            if self.place.bees[i].armor <= 0:
                self.place.remove_insect(self.place.bees[i])
        # END Problem 7
...
# END Problem 13
    ...
    def double_damage(self, cur_place, lst=[]):
        if cur_place.ant is not None and cur_place.ant not in lst:
            cur_place.ant.damage *= 2
            lst.append(cur_place.ant)
        if isinstance(cur_place.ant, BodyguardAnt) and cur_place.ant.ant is not None and cur_place.ant.ant not in lst:
            cur_place.ant.ant.damage *= 2
            lst.append(cur_place.ant.ant)
        if cur_place.exit is not None:
            self.double_damage(cur_place.exit, lst)
    ...
    def action(self, colony):
        ...
        if self.armor == 0:
            ...
            self.double_damage(self.place.exit)
        # END Problem 13
